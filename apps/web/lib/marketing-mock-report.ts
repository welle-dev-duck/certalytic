import type { CandidateReport } from "@/features/candidates/types";
...
export const marketingMockReport: CandidateReport = {
  ...
  componentSummaries: {
    ...
    s_int:
      "Round-two answers showed rehearsed phrasing and uneven depth on production debugging questions.",
    s_cross:
      "LinkedIn tenure for a senior role is shorter than the CV claims by approximately nine months.",
    s_id:
      "Identity signals are consistent across CV, transcript names, and public profile handles.",
  },
  componentIndicators: {
    s_cv: ["Stable employer sequence", "One overlapping date range"],
    s_int: [
      "Latency spikes on system design",
      "Generic architecture vocabulary",
    ],
    s_cross: [
      "LinkedIn role duration mismatch",
      "GitHub activity sparse in claimed period",
    ],
  ...
  platformMatrix: {
    linkedin_cv_match: {
      score: 48,
      explanation:
        "Senior title claimed on CV predates LinkedIn role start date.",
    },
    github_experience_match: {
      score: 55,
      explanation:
        "Commit frequency low during claimed intensive backend period.",
    },
    cross_platform_consistency: {
      score: 50,
      explanation:
        "Public profiles align on identity but diverge on tenure depth.",
    },
  },
  ...
  rounds: [
    {
      ...
      wasTruncated: false,
      observations: [
        "Strong on API fundamentals",
        "Specific latency numbers cited",
      ],
      deepDivePrompts: [
        "Walk through the caching layer failure you mentioned.",
      ],
    },
    {
      roundNumber: 2,
      ...
      wasTruncated: false,
      observations: [
        "Pivot to generic patterns under pressure",
        "Long pauses before system design answers",
      ],
      deepDivePrompts: ["Draw the exact queue topology from your last project."],
    },
  ],
  behaviourAnalysis: {
    summary:
    ...
    detail:
      "Concise in behavioural answers, formal and textbook-like when discussing architecture.",
    indicators: [
  ...
  personalityAnalysis: {
    ...
    detail:
      "Prefers structured problem decomposition and explicit success criteria before committing to designs.",
    indicators: [
