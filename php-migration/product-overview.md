...
**Stack:** Laravel 13 · Inertia.js (React 19) · Mistral AI (La Plateforme, EU) · Hetzner EU · Laravel Cashier (Stripe)  
**Status:** Production MVP — integrity screening, supplementary candidate insights, async role exports, audio transcription tooling, marketing site, and legal/compliance surfaces shipped
...
| **Team workspaces** | Multi-team model, invitations, seat-based plans, Stripe billing via Cashier |
...
| PDF generation | DomPDF (watermarked dossiers) |
| Object storage | Hetzner Object Storage (EU, S3-compatible) |
| Queue | Laravel Queue (Redis) — screenings, priority screenings, transcriptions, role PDF exports |
| Database | PostgreSQL |
| Compute | Hetzner Cloud + Coolify (EU) |
| Billing | Laravel Cashier + Stripe (billing only — no candidate PII in payments path) |
...
2. Record created; `ProcessCandidateScreeningJob` dispatched to Redis queue (priority queue on Scale+)
...
2. `role_exports` record created; `GenerateRoleExportPdfJob` queued
...
2. `TranscribeAudioJob` dispatched to `transcriptions` queue
...
Purchasable from Tools → Transcription via Stripe Checkout. Credited to `teams.transcript_tokens`.
...
Public route: `/` (`welcome` Inertia page).
...
Marketing stats are configurable via `CERTALYTIC_MARKETING_*` env vars (see §11).
...
Company and social links are configured via `CERTALYTIC_COMPANY_*` and `CERTALYTIC_SOCIAL_*` env vars, shared globally through Inertia.

## 9. Key database entities

- `users` — authentication (Fortify), current team selection
- `teams` — org workspace, plan, Stripe billing (Cashier), `transcript_tokens` balance
- `team_user` — membership, role (owner/member)
...
# Queue (required for async screening + transcription + role export jobs)
QUEUE_CONNECTION=redis
CERTALYTIC_QUEUE=default
CERTALYTIC_PRIORITY_QUEUE=screenings-priority
CERTALYTIC_TRANSCRIPTIONS_QUEUE=transcriptions

# Stripe / Cashier (billing + screening packs)
STRIPE_KEY=
STRIPE_SECRET=
...
# Company / legal (shared to Inertia as company + socialLinks)
...
Run `php artisan queue:work` (or `composer run dev`) so screenings, transcriptions, and role PDF exports process after submission.
